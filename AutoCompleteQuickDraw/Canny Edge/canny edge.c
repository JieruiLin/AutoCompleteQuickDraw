...
void canny_edge_detection(char* src, char* dst) {
	...
	non_maximum_suppression(nms[0], G, dir, png_get_rowbytes(png_read_ptr, read_info_ptr), png_get_image_height(png_read_ptr, read_info_ptr));
	hysteresis(final_output, nms[0], png_get_rowbytes(png_read_ptr, read_info_ptr), png_get_image_height(png_read_ptr, read_info_ptr), 105, 45);
...
void gaussian_filter(png_bytep *input, png_bytep *output, const unsigned width, const unsigned height, const float sigma) {
	...
	convolution(input[0], output, kernel, width, height, n, true);
}
...
//if (normalize) {
				for (int m = half; m < width - half; m++) {
								for (int n = half; n < height - half; n++) {
												...
												size_t c = 0;
												if(half==1){
																pixel += input[(n+1)*width + (m+1)] * kernel[0]+
																				input[n*width+ (m+1)] * kernel[1]+
																				input[(n-1)*width+ (m+1)] * kernel[2]+
																				input[(n+1)*width+ m] * kernel[3]+
																				input[n * width+ m] * kernel[4]+
																				input[(n-1)* width+ m] * kernel[5]+
																				input[(n+1)* width+ (m-1)] * kernel[6]+
																				input[n*width+ (m-1)] * kernel[7]+
																				input[(n-1)*width+ (m-1)] * kernel[8];
												}else{
																for (int i = -half; i <= half; i++) {
																				for (int j = -half; j <= half; j++) {
																								pixel += input[(n - j) * width + m - i] * kernel[c];
																								c++;
																				}
																}
												}
        	...
        	for (int m = half; m < width - half; m++) {
                for (int n = half; n < height - half; n++) {
                      ...
                      output[n][m] = (png_byte) (MAX_BRIGHTNESS * (output[n][m] - min) / (max - min));
...
void intensity_gradients(png_bytep *output, png_bytep *Gx_applied, png_bytep *Gy_applied, float *G, float *dir, const unsigned width, const unsigned height) {
	...
	convolution(output[0], Gx_applied, Gx, width, height, 3, false);
	convolution(output[0], Gy_applied, Gx, width, height, 3, false);
...
void non_maximum_suppression(png_bytep nms, float *G, float *dir, const unsigned width, const unsigned height) {
	...
	for (int i = 1; i < width - 1; i++) {
		for (int j = 1; j < height - 1; j++) {
			...
			int se = ss - 1;
			float Gc=G[c];
			if ((dir[c] <= 1 || dir[c] > 7) && Gc > G[ee] && Gc > G[ww]){
				nms[j*width+i]=Gc;
			}else if ((dir[c] > 1 && dir[c] <= 3) && Gc > G[nw] && Gc > G[se]) {
				nms[j*width+i] = Gc;
			} else if ((dir[c] > 3 && dir[c] <= 5) && Gc > G[nn] && Gc > G[ss]) {
				nms[j*width+i] = Gc;
			} else if ((dir[c] > 5 && dir[c] <= 7) && Gc > G[ne] && Gc > G[sw]) {
				nms[j*width+i] = Gc;
			} else {
				nms[j*width+i] = 0;
			}
...
void hysteresis(png_bytep *out, png_bytep nms, const unsigned width, const unsigned height, const unsigned tmax, const unsigned tmin) {
	...
	*/
	for (int j = 1; j < height - 1; j++) {
		for (int i = 1; i < width - 1; i++) {
			int c = i + width * j;
			if (nms[c] >= tmax && out[c / width][c % width] == 0) {
				...
				do {
					...
					for (int k = 0; k < 8; k+=4) {
						if (nms[nbs[k]] >= tmin && out[nbs[k]/ width][nbs[k] % width] == 0) {
							out[nbs[k]/ width][nbs[k] % width] = MAX_BRIGHTNESS;
							edges[nedges] = nbs[k];
							nedges++;
						}
						if (nms[nbs[k+1]] >= tmin && out[nbs[k+1]/ width][nbs[k+1] % width] == 0) {
							out[nbs[k+1]/ width][nbs[k] % width] = MAX_BRIGHTNESS;
							edges[nedges] = nbs[k+1];
							nedges++;
						}
						if (nms[nbs[k+2]] >= tmin && out[nbs[k+2]/ width][nbs[k+2] % width] == 0) {
							out[nbs[k+2]/ width][nbs[k+2] % width] = MAX_BRIGHTNESS;
							edges[nedges] = nbs[k+2];
							nedges++;
						}
						if (nms[nbs[k+3]] >= tmin && out[nbs[k+3]/ width][nbs[k+3] % width] == 0) {
							out[nbs[k+3]/ width][nbs[k+3] % width] = MAX_BRIGHTNESS;
							edges[nedges] = nbs[k+3];
							nedges++;
						}
					}
				} while (nedges > 0);
...
void allocate_read_mem(png_structp png_read_ptr, png_bytep *row_pointers, unsigned height, unsigned width) {
	png_byte *temp=png_malloc(png_read_ptr,width*height);
	//#pragma omp parallel for
	...
	for(unsigned row=0;row<height;row++){
		row_pointers[row]=& temp[row*width];
	}
}
...
void allocate_write_mem(png_structp png_write_ptr, png_bytep *rows1, png_bytep *rows2, png_bytep *rows3, png_bytep *rows4, png_bytep *rows5, unsigned height, unsigned width) {
	png_byte *temp1=png_malloc(png_write_ptr, height*width);
	png_byte *temp2=png_malloc(png_write_ptr, height*width);
	png_byte *temp3=png_malloc(png_write_ptr, height*width);
	png_byte *temp4=png_malloc(png_write_ptr, height*width);
	png_byte *temp5=png_malloc(png_write_ptr, height*width);
	...
	//#pragma omp parallel for
	for(unsigned row=0;row<height;row++){
		rows1[row]=&temp1[row*width];
		rows2[row]=&temp2[row*width];
		rows3[row]=&temp3[row*width];
		rows4[row]=&temp4[row*width];
		rows5[row]=&temp5[row*width];
	}
}
...
void cleanup_rows(png_structp png_write_ptr, png_bytep *rows1, png_bytep *rows2, png_bytep *rows3, png_bytep *rows4, png_bytep *rows5, png_structp png_read_ptr, png_bytep *readrows, unsigned height) {
		png_free(png_write_ptr, rows1[0]);
		png_free(png_write_ptr, rows2[0]);
		png_free(png_write_ptr, rows3[0]);
		png_free(png_write_ptr, rows4[0]);
		png_free(png_write_ptr, rows5[0]);
		png_free(png_read_ptr, readrows[0]);
